/**
 * Cliente MQTT - Publisher:/Temperatura; Subscribed:/led
 * 
...
// --- Definições de pinos conforme flood_alert.c ---
...
#define WIFI_SSID "wifi_id"                  // Substitua pelo nome da sua rede Wi-Fi
...
#define MQTT_USERNAME "user"     // Substitua pelo nome da host MQTT - Username
#define MQTT_PASSWORD "senha"     // Substitua pelo Password da host MQTT - credencial de acesso - caso exista
...
// --- Funções utilitárias para LED RGB e Buzzer ---
static void _pwm_init(int pin) {
...
static void set_rgb_led_red() {
    gpio_put(LED_R, 1);
    gpio_put(LED_G, 0);
    gpio_put(LED_B, 0);
}

static void set_rgb_led_off() {
    gpio_put(LED_R, 0);
    gpio_put(LED_G, 0);
    gpio_put(LED_B, 0);
}
...
// --- Leitura do joystick ---
static float read_joystick(uint8_t channel) {
    ...
    media /= 100.0f;
    return media * (100.0f / 4095.0f); // valor percentual
}

// --- Publicação dos valores do joystick ---
static void publish_joystick(MQTT_CLIENT_DATA_T *state) {
    static float old_x = -1, old_y = -1;
    float x = read_joystick(JOYSTICK_X_ADC);
    float y = read_joystick(JOYSTICK_Y_ADC);

    if (x != old_x) {
        old_x = x;
        char x_str[16];
        snprintf(x_str, sizeof(x_str), "%.2f", x);
        mqtt_publish(state->mqtt_client_inst, full_topic(state, "/chuva"), x_str, strlen(x_str), MQTT_PUBLISH_QOS, MQTT_PUBLISH_RETAIN, pub_request_cb, state);
    }
    if (y != old_y) {
        old_y = y;
        char y_str[16];
        snprintf(y_str, sizeof(y_str), "%.2f", y);
        mqtt_publish(state->mqtt_client_inst, full_topic(state, "/agua"), y_str, strlen(y_str), MQTT_PUBLISH_QOS, MQTT_PUBLISH_RETAIN, pub_request_cb, state);
    }
}

// --- Worker para publicar joystick periodicamente ---
static void joystick_worker_fn(async_context_t *context, async_at_time_worker_t *worker) {
    MQTT_CLIENT_DATA_T* state = (MQTT_CLIENT_DATA_T*)worker->user_data;
    publish_joystick(state);
    async_context_add_at_time_worker_in_ms(context, worker, TEMP_WORKER_TIME_S * 1000);
}
static async_at_time_worker_t joystick_worker = { .do_work = joystick_worker_fn };

// --- Modifique os tópicos de assinatura ---
static void sub_unsub_topics(MQTT_CLIENT_DATA_T* state, bool sub) {
    mqtt_request_cb_t cb = sub ? sub_request_cb : unsub_request_cb;
    mqtt_sub_unsub(state->mqtt_client_inst, full_topic(state, "/alerta"), MQTT_SUBSCRIBE_QOS, cb, state, sub);
}
...
static void mqtt_incoming_data_cb(void *arg, const u8_t *data, u16_t len, u8_t flags) {
    ...
    if (strcmp(basic_topic, "/alerta") == 0) {
        if (strcmp((const char *)state->data, "1") == 0) {
            set_rgb_led_red();
            buzzer_on();
        } else {
            set_rgb_led_off();
...
int main(void) {
    ...
    setup_alert_gpio(); // Inicializa GPIOs do LED RGB e buzzer
...
static void mqtt_connection_cb(mqtt_client_t *client, void *arg, mqtt_connection_status_t status) {
    ...
    if (status == MQTT_CONNECT_ACCEPTED) {
        ...
        // Inicia publicação periódica do joystick
        joystick_worker.user_data = state;
        async_context_add_at_time_worker_in_ms(cyw43_arch_async_context(), &joystick_worker, 0);
